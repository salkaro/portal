...
type ActivityEvent = {
  id: string;
  type: "completed" | "in_progress" | "overdue" | "added";
  label: string;
  itemName: string;
  dateText: string;
  sortDate: Date;
...
function getItemDate(item: PortalItem): Date | null {
  ...
  const dateStr = dateCol.text.includes(" - ") ? dateCol.text.split(" - ")[0] : dateCol.text;
...
function deriveEvents(items: PortalItem[]): ActivityEvent[] {
  ...
  for (const item of items) {
    ...
    if (isStatusDone(statusText)) {
      events.push({ id: `done-${item.id}`, type: "completed", label: "Completed", itemName: item.name, dateText: date ? formatEventDate(date) : "Recently", sortDate });
    } else if (isStatusInProgress(statusText)) {
      events.push({ id: `progress-${item.id}`, type: "in_progress", label: "In Progress", itemName: item.name, dateText: date ? formatEventDate(date) : "Active", sortDate });
    } else if (date && date < now) {
      events.push({ id: `overdue-${item.id}`, type: "overdue", label: "Needs Attention", itemName: item.name, dateText: formatEventDate(date), sortDate });
    } else {
      events.push({ id: `added-${item.id}`, type: "added", label: "Scheduled", itemName: item.name, dateText: date ? formatEventDate(date) : "Planned", sortDate });
    }
  }

  const typeOrder: Record<ActivityEvent["type"], number> = { completed: 0, in_progress: 1, overdue: 2, added: 3 };
...
const eventConfig: Record<ActivityEvent["type"], { Icon: React.ElementType; iconClass: string; labelClass: string }> = {
