...
export async function exportPortalPdf(
  ...
  // Dynamic import so jsPDF is never bundled on the server
  ...
  function pill(text: string, x: number, py: number, color: [number, number, number]) {
  ...
  doc.text(`Exported ${dateStr}`, pageW - margin, 21, { align: "right" });
  ...
  const cards: { label: string; value: string | number; color: [number, number, number] }[] = [
    ...
    { label: "Overall", value: `${stats.donePercent}%`, color: COLORS.accent },
    { label: "Overdue", value: overdue, color: overdue > 0 ? COLORS.red : COLORS.muted },
  ];

  const cardW = (contentW - 4 * 4) / 5; // 5 cards, 4 gaps of 4mm
  ...
  const visibleColumns = data.columns.slice(0, 4); // cap to avoid overflowing columns
  ...
  for (const groupId of groupOrder) {
    ...
    const head = [
      ["Item", "Status", ...visibleColumns.map((c) => c.title)],
    ];
